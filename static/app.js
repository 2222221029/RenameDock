...
async function openDirectoryBrowser() {
  ...
  if (!showDialog(dialog)) {
    toast("目录浏览器无法打开，请刷新页面后重试", "error");
    return;
  }
  try {
    await browse($("#pathInput").value.trim() || state.roots[0]?.path || "/data");
  } catch (error) {
    toast(error.message || "无法读取目录", "error");
  }
}
...
async function bootstrap() {
  ...
  roots.disabled = true;
  roots.innerHTML = "<option>正在连接 RenameDock…</option>";
  ...
  try {
    ...
    roots.innerHTML = "<option>挂载目录加载失败</option>";
    roots.disabled = true;
    $("#bootstrapErrorText").textContent = `初始化失败：${error.message}`;
...
function previewPageRange() {
  const pageSize = state.previewShowAll ? Math.max(1, state.items.length) : state.previewPageSize;
  const pageCount = Math.max(1, Math.ceil(state.items.length / pageSize));
  state.previewPage = Math.min(Math.max(0, state.previewPage), pageCount - 1);
  const start = state.previewPage * pageSize;
  const end = Math.min(state.items.length, start + pageSize);
  return {start, end, pageCount};
}

function updatePreviewPager(range = previewPageRange()) {
  const pager = $("#previewPager");
  const pageInfo = $("#previewPageInfo");
  const previous = $("#previewPrev");
  const next = $("#previewNext");
  const showAll = $("#previewShowAll");
  if (!pager || !pageInfo || !previous || !next || !showAll) return;
  pager.classList.toggle("hidden", !state.items.length);
  pageInfo.textContent = `${state.previewPage + 1} / ${range.pageCount}`;
  previous.disabled = !state.items.length || state.previewPage === 0;
  next.disabled = !state.items.length || state.previewPage >= range.pageCount - 1;
  showAll.textContent = state.previewShowAll ? "分页显示" : "显示全部";
  previous.classList.toggle("hidden", state.previewShowAll);
  pageInfo.classList.toggle("hidden", state.previewShowAll);
  next.classList.toggle("hidden", state.previewShowAll);
}

function updatePreviewHint(range = previewPageRange()) {
  updatePreviewPager(range);
  ...
  $("#previewHint").textContent = `${state.items.length} 项 · ${first}–${range.end} · ${state.rules.length} 条规则`;
}
...
function changePreviewPage(delta) {
  const range = previewPageRange();
  const next = Math.min(Math.max(0, state.previewPage + delta), range.pageCount - 1);
...
function togglePreviewShowAll() {
  state.previewShowAll = !state.previewShowAll;
  state.previewPage = 0;
  $(".table-wrap").scrollTop = 0;
  renderPreview();
}
...
function bindEvents() {
  ...
  $("#previewPrev")?.addEventListener("click", () => changePreviewPage(-1));
  $("#previewNext")?.addEventListener("click", () => changePreviewPage(1));
  $("#previewShowAll")?.addEventListener("click", togglePreviewShowAll);
  ...
  $("#browseButton").addEventListener("click", event => { event.preventDefault(); openDirectoryBrowser(); });
  $("#pathInput").addEventListener("dblclick", openDirectoryBrowser);
